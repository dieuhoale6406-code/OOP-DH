...
static bool mssvHopLe(const String& s){
    ...
    for(int i = 0; i < 10; i++) if(!is_digit(s[i])) return false;
    return true;
}
...
SinhVien::SinhVien() : Person(), mssv("1000000000"), gpa(0.0), Class(nullptr) {}

SinhVien::SinhVien(const String& name, int age, const String& address, bool gender, const String& id, double gpa)
    : Person(name, age, address, gender), mssv("1000000000"), gpa(0.0), Class(nullptr) {
    setId(id);
    setGPA(gpa);
}
...
void SinhVien::setClass(ClassSH* Class){
    this->Class = Class;
}
...
istream& operator>>(istream& is, SinhVien& sv) {
    ...
    while(true) {
        ...
        try {
            ...
            cout << "Loi: " << e.what() << ". Vui long nhap lai" << endl;
        }
    }
